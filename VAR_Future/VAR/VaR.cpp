...
void VaR::historical() {
	...
	vector<double> PL_dollar, PL_percentage;
	...
	cout << "Price" << setw(15) << "Portfolio" << setw(15) << "P&L" << setw(15) << "P&L(%)" <<endl;
	for (int i = 1; i < data.rowCount() - 1; i++) {
		double NewPrice = stod(data[i]["Price"]);
		double SimulatedPrice = NewPrice - OldPrice + SpotPrice;
		double pl_return = stod(data[i]["Return"])*100;
		OldPrice = NewPrice;
		double port = SimulatedPrice * quantity;
		double pl = port - oldPV;
		PL_dollar.push_back(pl);
		PL_percentage.push_back(pl_return);
		//cout << SimulatedPrice << setw(15) << port << setw(15) << pl << setw(15) << pl_return << endl;
	}
	cout << "The one day 95% dollar-VaR based on Historical method is: " << risk_VaR(PL_dollar, 5) <<"$"<< endl;
	//cout << "The one day 95% percentage-VaR based on Historical method is: " << risk_VaR(PL_percentage, 5) <<"%"<< endl << endl;
}
